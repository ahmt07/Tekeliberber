...
function mesajYaz(string $tip, string $metin): void {
    $_SESSION['mesaj'] = ['tip' => $tip, 'metin' => $metin];
}

function mesajOku(): ?array {
    $m = $_SESSION['mesaj'] ?? null;
...
function girisZorunlu(): void {
    if (!girisVarMi()) {
        yonlendir('?sayfa=giris');
    }
}

function rolZorunlu(string $rol): void {
    girisZorunlu();
...
function durumMetni(string $durum): string {
    return match($durum) {
        'bekliyor' => 'Bekliyor',
        'onaylandi' => 'Onaylandı',
...
function durumSinifi(string $durum): string {
    return match($durum) {
        'bekliyor' => 'bekliyor',
        'onaylandi' => 'onaylandi',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($islem === 'kayit_ol') {
        ...
        $telefon = telefonTemizle($_POST['telefon'] ?? '');
        $sifre = (string)($_POST['sifre'] ?? '');

        if ($adSoyad === '' || strlen($telefon) < 10 || strlen($sifre) < 6) {
            mesajYaz('hata', 'Bilgileri eksiksiz gir. Şifre en az 6 karakter olmalı.');
            yonlendir('?sayfa=kayit');
        }

        $kontrol = $db->prepare("SELECT COUNT(*) FROM kullanicilar WHERE telefon = ?");
        $kontrol->execute([$telefon]);
        if ((int)$kontrol->fetchColumn() > 0) {
            mesajYaz('hata', 'Bu telefon ile kayıt zaten var.');
            yonlendir('?sayfa=kayit');
        }
    ...
    if ($islem === 'giris_yap') {
        ...
        mesajYaz('basarili', 'Giriş başarılı.');
        yonlendir('?sayfa=panel');
    }
    ...
    if ($islem === 'cikis_yap') {
        session_destroy();
        session_start();
        mesajYaz('basarili', 'Çıkış yapıldı.');
        yonlendir('?sayfa=giris');
    }
    ...
    if ($islem === 'randevu_olustur') {
        ...
        if (ayniSaatDoluMu($db, $berberId, $tarih, $saat)) {
            mesajYaz('hata', 'Bu saat dolu. Başka saat seç.');
            yonlendir('?sayfa=panel');
        }
    ...
    if ($islem === 'randevu_durum_degistir') {
        ...
        mesajYaz('basarili', 'Randevu durumu güncellendi.');
        yonlendir('?sayfa=panel');
    }
    ...
    if ($islem === 'hizmet_ekle') {
        ...
        if ($ad === '' || $sure <= 0 || $fiyat <= 0) {
            mesajYaz('hata', 'Hizmet bilgileri hatalı.');
            yonlendir('?sayfa=panel#hizmetler');
        }

        $s = $db->prepare("INSERT INTO hizmetler (ad, sure_dk, fiyat, aktif) VALUES (?, ?, ?, 1)");
        $s->execute([$ad, $sure, $fiyat]);

        mesajYaz('basarili', 'Hizmet eklendi.');
        yonlendir('?sayfa=panel#hizmetler');
    }
    ...
    if ($islem === 'berber_ekle') {
        ...
        if ($adSoyad === '' || strlen($telefon) < 10 || strlen($sifre) < 6) {
            mesajYaz('hata', 'Berber bilgileri eksik.');
        ...
        $kontrol->execute([$telefon]);
        if ((int)$kontrol->fetchColumn() > 0) {
            mesajYaz('hata', 'Bu telefon zaten kayıtlı.');
            yonlendir('?sayfa=panel#berberler');
        }
...
.logo{
    ...
    color:var(--altin2);font-weight:800
}
...
.menu a{
    ...
    border:1px solid transparent;color:#efe5d3
}
...
input,select,textarea{
    ...
    border:1px solid var(--kenar);background:#120f0f;color:#fff;outline:none
}
...
.koyu{
    background:#1f1919;color:#f3eadc;border:1px solid var(--kenar)
}
...
.sekme{
    padding:10px 14px;border-radius:999px;border:1px solid var(--kenar);
...
.tablo th,.tablo td{
    text-align:left;padding:12px;border-bottom:1px solid rgba(212,175,55,.1);vertical-align:top
}
    ...
    <main class="ana">
        <div class="ust">
            <div>
                ...
                <h2><?= e(kullanici()['ad_soyad']) ?></h2>
            </div>
            <div class="kutu" style="padding:12px 16px">
                <b><?= e(kullanici()['ad_soyad']) ?></b><br>
                <span class="kucuk"><?= e(rolMetni(kullanici()['rol'])) ?></span>
            </div>
        </div>
        ...
        <?php if (kullanici()['rol'] === 'admin'): ?>
            ...
            <section id="admin-randevular" class="kutu" style="margin-top:18px">
                ...
                <div class="liste">
                    ...
                    <?php else: foreach ($bugunkuListe as $r): ?>
                    <div class="satir">
...
<script>
(function(){
    var hedef = document.getElementById('qrcode');
    if (hedef && window.QRCode) {
        new QRCode(hedef, {
